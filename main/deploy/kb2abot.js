			...
			if (permission == 'admin') {
				try {
					...
					if (
						...
						return reply('Tuổi gì dùng lệnh của admin!');
				} catch (e) {
					reply(
		...
		if (keyword) {
			if (keyword.includes('.')) {
				...
				if (found.length == 0) {
					...
					reply(
						`Làm gì có lệnh: "${keyword}"\nCác lệnh gần giống nè: ${bestMatches.join(
							', '
						)}\nXem lại danh sách lệnh đi ${thread.storage.prefix}help!`
					);
				}
